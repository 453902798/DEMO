...
y2 = y_hybrid_vir;
% 最终混合预测值
w31 = 1./(mean4)^2./(1./(mean4)^2+1./(mean8)^2);w32 = 1./(mean8)^2./(1./(mean4)^2+1./(mean5)^8);
y_hybrid_hybrid = w31*y1 + w32*y2;
error_hybrid_hybrid = abs(y_hybrid_hybrid - y_test); mean9 = mean(error_hybrid_hybrid);
% w31 = 1./(mean1)^2./(1./(mean1)^2+1./(mean5)^2);w32 = 1./(mean5)^2./(1./(mean1)^2+1./(mean5)^2);
% w41 = 1./(mean2)^2./(1./(mean2)^2+1./(mean6)^2);w42 = 1./(mean6)^2./(1./(mean2)^2+1./(mean5)^6);
% w51 = 1./(mean3)^2./(1./(mean3)^2+1./(mean7)^2);w52 = 1./(mean7)^2./(1./(mean3)^2+1./(mean5)^7);
% y_Kriging = w31*value_Kriging + w32*value_Kriging_vir;
% y_RBF = w41*value_RBF + w42*value_RBF_vir;
% y_SVR = w51*value_SVR + w52*value_SVR_vir;
% error_Kriging = abs(y_Kriging - y_test);error_RBF = abs(y_RBF - y_test);error_SVR = abs(y_SVR - y_test);
% mean_Kriging = mean(error_Kriging);mean_RBF = mean(error_RBF);mean_SVR = mean(error_SVR);
% w61 = 1./(mean_Kriging)^2./(1./(mean_Kriging)^2+1./(mean_RBF)^2+1./(mean_SVR)^2);
% w62 = 1./(mean_RBF)^2./(1./(mean_Kriging)^2+1./(mean_RBF)^2+1./(mean_SVR)^2);
% w63 = 1./(mean_SVR)^2./(1./(mean_Kriging)^2+1./(mean_RBF)^2+1./(mean_SVR)^2);
% y_hybrid_hybrid = w61*y_Kriging + w62*y_RBF + w63*y_SVR;
% error_hybrid_hybrid = abs(y_hybrid_hybrid - y_test); mean9 = mean(error_hybrid_hybrid);
% 
% %% 6 混合数据后预测
