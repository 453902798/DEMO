...
os.chdir(r"D:/ABAQUS/ABAQUS_practice/GraduateDesign/2_60/data/1ball")#修改工作目录，用于最后保存odb文件
filePath = "D:/ABAQUS/ABAQUS_practice/GraduateDesign/2_60/data/1ball/"#cvs文件路径
fr = open(filePath+"graduatedesign_1_2.csv",'r')#csv文件名
...
#########################
    ...
    #3.4芯棒调整
    a = mdb.models['Model-0'].rootAssembly
    a.rotate(instanceList=('ball1-1', 'shank-1'), axisPoint=(
        0.0, 0.0, 0.0), axisDirection=(0.0, 1.0, 0.0), angle=90.0)
    #: The instances were rotated by 90. 度(关于由点 0., 0., 0. 和向量 0., 1., 0. 定义的轴线)
    a = mdb.models['Model-0'].rootAssembly
    a.translate(instanceList=('ball1-1', 'shank-1'), vector=(
        0.0, radius, prolongationofshank))
    #: The instances were translated by 0., 弯曲半径, 初始伸长量(相对于装配坐标系)
    a = mdb.models['Model-0'].rootAssembly
    a.translate(instanceList=('ball1-1',), vector=(0.0, 0.0, spacingofmandrel+widthofmandrel/2-prolongationofshank))
...
#创建连接线
    ...
    a.WirePolyLine(points=((r11[2], r12[31]),), mergeType=IMPRINT, meshable=OFF)
    a = mdb.models['Model-0'].rootAssembly
    e1 = a.edges
    edges1 = e1.getSequenceFromMask(mask=('[#1 ]',), )
    ...
    datum1 = mdb.models['Model-0'].rootAssembly.datums[29]
...
mdb.saveAs(pathName='D:/ABAQUS/ABAQUS_practice/GraduateDesign/2_60/data/1ball'+paralist[i+2][0])#mdb文件保存路径及文件名
