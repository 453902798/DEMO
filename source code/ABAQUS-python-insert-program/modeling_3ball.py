...
os.chdir(r"D:/ABAQUS/ABAQUS_practice/GraduateDesign/2_60/data/3ball")#修改工作目录，用于最后保存odb文件
filePath = "D:/ABAQUS/ABAQUS_practice/GraduateDesign/2_60/data/3ball/"#cvs文件路径
fr = open(filePath+"graduatedesign_3_1.csv",'r')#csv文件名
...
#########################
    ...
    #3.4芯棒调整
    a = mdb.models['Model-0'].rootAssembly
    a.rotate(instanceList=('ball1-1', 'ball2-1', 'ball3-1', 'shank-1'), axisPoint=(
        0.0, 0.0, 0.0), axisDirection=(0.0, 1.0, 0.0), angle=90.0)
    #: The instances were rotated by 90. 度(关于由点 0., 0., 0. 和向量 0., 1., 0. 定义的轴线)
    a = mdb.models['Model-0'].rootAssembly
    a.translate(instanceList=('ball1-1', 'ball2-1', 'ball3-1', 'shank-1'), vector=(
    ...
    #: The instance ball3-1 was translated by 0., 0., 26. (相对于装配坐标系)
...
#创建连接线
    ...
    a.WirePolyLine(points=((r11[2], r12[36]),), mergeType=IMPRINT, meshable=OFF)
    a = mdb.models['Model-0'].rootAssembly
    e1 = a.edges
    edges1 = e1.getSequenceFromMask(mask=('[#1 ]',), )
    ...
    r12 = a.referencePoints
    a.WirePolyLine(points=((r11[2], r12[37]),), mergeType=IMPRINT, meshable=OFF)
    a = mdb.models['Model-0'].rootAssembly
    e1 = a.edges
    edges1 = e1.getSequenceFromMask(mask=('[#1 ]',), )
    ...
    r11 = a.instances['ball2-1'].referencePoints
    r12 = a.referencePoints
    a.WirePolyLine(points=((r11[2], r12[38]),), mergeType=IMPRINT, meshable=OFF)
    a = mdb.models['Model-0'].rootAssembly
    e1 = a.edges
    edges1 = e1.getSequenceFromMask(mask=('[#1 ]',), )
    ...
    datum1 = mdb.models['Model-0'].rootAssembly.datums[33]
    ...
    datum1 = mdb.models['Model-0'].rootAssembly.datums[34]
    csa = a.SectionAssignment(sectionName='ConnSect-3', region=region)
    #: 截面 "ConnSect-3" 已指派给 1 条线或附着线.
    a.ConnectorOrientation(region=csa.getSet(), localCsys1=datum1)
    a = mdb.models['Model-0'].rootAssembly
...
mdb.saveAs(pathName='D:/ABAQUS/ABAQUS_practice/GraduateDesign/2_60/data/3ball'+paralist[i+2][0])#mdb文件保存路径及文件名
